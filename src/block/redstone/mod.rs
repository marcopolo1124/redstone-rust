...
pub fn set_power_to_0(
    ...
    let values = match *blk {
        ...
        Some(Block { kind: BlockKind::Mechanism { .. }, .. }) => {
            listeners.mechanism_off.insert((x, y));
            None
        }
    ...
    for (next_x, next_y) in next_coord {
        set_power_to_0(map, next_x, next_y, signal_type, curr_signal, listeners, traversed);
    }
...
pub fn set_power(
    ...
    let values = match *blk {
        ...
        Some(Block { kind: BlockKind::Mechanism { .. }, .. }) => {
            listeners.mechanism_on.insert((x, y));
            None
        }
    ...
    match values {
        ...
        Some((signal, output_ports, signal_type)) => {
            ...
            for (next_x, next_y) in next_blocks {
                if signal > 1 {
                    set_power(map, next_x, next_y, signal - 1, signal_type, listeners, traversed);
                }
            }
        }
    }
        ...
        Some(SignalType::Strong(_)) | Some(SignalType::Weak(true)) | None => {
            match *kind {
                RedstoneKind::Torch => {
                    ...
                    } else if *signal > 0 && input_signal > 0 {
                        listeners.redstone_torch_off.insert((x, y));
                        None
                    } else {
                        None
                    }
    ...
    match signal_type {
        Some(SignalType::Strong(_)) | Some(SignalType::Weak(true)) | None => {
            ...
            match *kind {
                RedstoneKind::Torch => {
                    if prev_signal < 20 {
                        listeners.redstone_torch_on.insert((x, y));
                    } else {
